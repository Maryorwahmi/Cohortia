...
const AUTOMATION_JOB_STORAGE_KEY = 'cohortia-automation-job';
...
export default function AutomationPage() {
  ...
  useEffect(() => {
    try {
      const savedJob = localStorage.getItem(AUTOMATION_JOB_STORAGE_KEY);
      if (!savedJob) return;
      const parsed = JSON.parse(savedJob) as { jobId?: string; category?: string; selectedCourse?: string; logs?: string[] };
      if (parsed?.jobId) {
        ...
        if (Array.isArray(parsed.logs)) {
          setLiveLogs(parsed.logs.slice(-80));
        }
        setMessage('Resuming the saved automation job…');
      }
    } catch {
      localStorage.removeItem(AUTOMATION_JOB_STORAGE_KEY);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const persistJob = (logs: string[] = liveLogs) => {
      try {
        localStorage.setItem(
          AUTOMATION_JOB_STORAGE_KEY,
          JSON.stringify({
            jobId,
            category,
            selectedCourse,
  ...
  }, [jobId, category, selectedCourse, liveLogs]);
    ...
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          jobId: nextJobId,
          category,
          selectedCourse: selectedCourse === ALL_SUBCATEGORY_OPTION ? selectedSubcategory : selectedCourse,
          logs: [],
          updatedAt: new Date().toISOString(),
        }),
      );
