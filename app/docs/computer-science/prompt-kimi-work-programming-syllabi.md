# Copilot Work Prompt — Generate Cohortia Computer Science Syllabi

> **Use this entire document as the prompt in Copilot Work.**  
> **Goal:** Create detailed, chapter-level syllabi for every course in the Cohortia Computer Science catalog so that user can generate slides, audio, video, quizzes, and coding exercises automatically.
...
**What we are doing now:** We are moving from catalog research to full syllabus design. We create one detailed syllabus file per course/certificate. Each syllabus breaks the course into **modules** and **chapters**. Every chapter must contain a detailed **AI generation note** so we knows what kind of lesson to create.

**Current progress:**
- ✅ **Arduino Programming** — completed (Beginner)  
  `docs/computer-science/beginner/hardware-embedded-iot/arduino-programming.md`
- ✅ **Raspberry Pi for Beginners** — completed (Beginner)  
  `docs/computer-science/beginner/hardware-embedded-iot/raspberry-pi-for-beginners.md`
- ✅ **Computer Architecture** — completed (Advanced, 12 modules, ~85 chapters)  
  `docs/computer-science/advanced/hardware-embedded-iot/computer-architecture.md`
- ✅ **From Nand to Tetris** — completed (Intermediate, 8 modules, 48 chapters)  
  `docs/computer-science/intermediate/hardware-embedded-iot/from-nand-to-tetris.md`
- 🟡 **Currently working on:** the remaining **Hardware, Embedded & IoT** courses, then the remaining 12 subcategories.

These four completed files define the new standard. They are very detailed, with real explanatory content in every module and chapter, not bullet-point outlines. Copy their depth, tone, and structure for every future syllabus.
...
Do not write chapter headings with only bullet lists. Write full explanations, definitions, code examples, circuit descriptions, analogies, common mistakes, and practical activities.
...
- If a course is known to be extensive (e.g., the original Nand2Tetris has 14 major projects, or a specialization with 5+ courses), use the **upper end** of the range or even exceed it if necessary.
...
|---|-------------|---------|--------|
| 1 | Programming & Software Development Fundamentals | 40 | Not started |
| 2 | Web Development | 31 | Not started |
...
| 13 | Hardware, Embedded & IoT | 11 | 4 done, 5 remaining |

**Total: 338 courses.** The long-term goal is a full syllabus for every course.

---

## 6. Current Focus: Hardware, Embedded & IoT (5 Remaining)

We are finishing the **Hardware, Embedded & IoT** subcategory. The catalog lists 11 entries, but 2 are meta/admin entries (`verify-uncertain-degrees` and `decide-on-inclusion-rules`) and should be skipped.

### Courses already completed
- `arduino-programming` — Arduino Programming (Beginner)
- `raspberry-pi-for-beginners` — Raspberry Pi for Beginners (Beginner)
- `computer-architecture` — Computer Architecture (Advanced, 12 modules, ~85 chapters)
- `from-nand-to-tetris` — From Nand to Tetris (Intermediate, 8 modules, 48 chapters)

### Courses still to write (NEXT TO WORK ON)
1. **`embedded-systems-essentials`** — Embedded Systems Essentials (UT Austin / edX) — Beginner level
2. **`fpga-design-for-embedded-systems`** — FPGA Design for Embedded Systems (CU Boulder / Coursera) — Intermediate/Advanced
3. **`an-introduction-to-programming-the-internet-of-things-iot`** — An Introduction to Programming the Internet of Things (IoT) (UC Irvine / Coursera) — Beginner/Intermediate
4. **`robotics-specialization`** — Robotics Specialization (UPenn / Coursera) — Intermediate/Advanced
5. **`ros-for-beginners`** — ROS for Beginners (Udemy) — Beginner

After these 5 are done, move to the next subcategory. The recommended order is:

1. Hardware, Embedded & IoT (finish current — 5 remaining)
2. Programming & Software Development Fundamentals
3. Web Development
...
9. Data Science, Machine Learning & AI
10. Cybersecurity
11. Cloud Computing & DevOps
12. Game Development
13. Blockchain & Web3

You may adjust the order, but always finish one subcategory before jumping to another unless instructed otherwise.
...
Use the **four completed files** as your template:
...
- **Real code examples.** Include actual code snippets in the correct language for the course.
- **Real commands and tools.** For hardware/Linux courses, include real terminal commands, wiring descriptions, and library names.
...
- **Visual style:** animations, live coding, diagrams, side-by-side comparisons, circuit wiring, oscilloscope-style waveforms, real hardware demos.
- **Examples to use:** real-world analogies, specific code snippets, specific datasets, specific hardware components.
- **Interactive element:** one coding exercise, one reflection prompt, one drag-and-drop, one mini-quiz, or one hands-on lab step.
- **Accessibility requirements:** captions, alt text for diagrams, transcripts, high-contrast visuals.

**Example from the Arduino pilot:**
...
3. Look for:
   ...
   - Required tools, languages, hardware, or software
...
1. Read the **four completed pilot syllabi** to internalize the new standard:
   - `docs/computer-science/beginner/hardware-embedded-iot/arduino-programming.md`
   - `docs/computer-science/beginner/hardware-embedded-iot/raspberry-pi-for-beginners.md`
   - `docs/computer-science/intermediate/hardware-embedded-iot/from-nand-to-tetris.md`
   - `docs/computer-science/advanced/hardware-embedded-iot/computer-architecture.md`
2. Read the catalog source of truth:
   - `docs/computer-science/catalog-courses-by-subcategory.json`
3. Continue with the remaining **Hardware, Embedded & IoT** courses (5 remaining), then move to the next subcategory.
