 ...
 *
 * Cohortia Syllabus Generator — Chunked, multi-key, subcategory-aware
...
const CATALOG_PATH = path.join(REPO_ROOT, 'docs', 'computer-science', 'catalog-courses-by-subcategory.json');
const STATE_PATH = path.join(REPO_ROOT, 'scripts', '.syllabus-generation-state.json');
const DEFAULT_PROMPT_FILE_PATH = path.join(REPO_ROOT, 'docs', 'computer-science', 'prompt-kimi-work-computer-science-syllabi.md');
...
function getPromptFilePath(subcategoryName) {
  ...
  const slugToPromptFile = {
    'programming-software-development-fundamentals': 'prompt-kimi-work-programming-syllabi.md',
  };
  ...
  const candidates = mappedFileName
    ? [path.join(REPO_ROOT, 'docs', 'computer-science', mappedFileName)]
    : [];
  candidates.push(path.join(REPO_ROOT, 'docs', 'computer-science', `prompt-kimi-work-${slug}-syllabi.md`));
...
const PILOT_SYLLABI_DIR = path.join(REPO_ROOT, 'docs', 'computer-science');
...
function getOutputPath(course) {
  ...
  const targetDir = path.join(REPO_ROOT, 'docs', 'computer-science', getLevelFolder(course.level), safeSubcategory);
...
function buildDefaultStructure(course) {
  ...
  const isWebDevelopment = subcategory === 'web development';

  const stopWords = isWebDevelopment
    ? ['with', 'from', 'for', 'the', 'and', 'introduction', 'certified', 'programmer', 'programming', 'complete', 'fundamentals', 'basics', 'advanced', 'beginner', 'intermediate', 'developer', 'course', 'specialization', 'certificate', 'professional']
    : ['with', 'from', 'for', 'the', 'and', 'introduction', 'certified', 'professional', 'programmer', 'programming', 'complete', 'fundamentals', 'basics', 'advanced', 'beginner', 'intermediate', 'developer', 'course', 'specialization', 'certificate'];

  const topicWords = course.title.toLowerCase().split(/\s+/).filter((w) => w.length > 3 && !stopWords.includes(w) && !/^\d+$/.test(w));

  const mainTopic = topicWords[0] || (isWebDevelopment ? 'Web Development' : (course.subcategory || 'Computer Science'));
  ...
  if (isWebDevelopment) {
    templates = {
      ...
      'advanced': [
        `Advanced ${mainTopic} Architecture`,
        `Scalable Front-End Engineering`,
        `High-Performance Back-End Systems`,
        `Microservices and Serverless Architectures`,
        `Advanced Security and DevOps`,
        `Progressive Web Apps and Modern Platforms`,
    ...
    // Generic fallback for any Computer Science subcategory (Cybersecurity, Cloud,
    // Databases, Networks, Game Development, Blockchain, Mobile, etc.). These
    // templates avoid programming-specific language so they work safely across subjects.
    templates = {
      'beginner': [
        `Introduction to ${mainTopic}`,
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║     Cohortia Syllabus Generator — Gemini Edition             ║');
  console.log('╠══════════════════════════════════════════════════════════════╣');
  ...
  if (args.courseId) {
    ...
    if (!course) {
      ...
      const likelySubcategory = args.subcategory || 'Programming & Software Development Fundamentals';
